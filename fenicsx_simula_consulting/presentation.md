...
Find $\in V_0$ such that

$$
R(u_h, x)= - \nabla \cdot (\nabla u_h) - f(x) = 0  \text{ in } \Omega \\
$$

Select bases $\{\phi_i\}$ spanning $V_0$
...
$$
\begin{align*}
\langle R(u_h, x), \phi_j\rangle &= \int_\Omega -\nabla \cdot (\nabla u_h) \phi_j -f\phi_j~\mathrm{dx}\\
&=\int_\Omega \nabla u_h \cdot \nabla \phi_j - f\phi_j~\mathrm{d}x - \int_{\partial\Omega}\frac{\partial u}{\partial n} \phi_j~\mathrm{d}x=0
\end{align*}
$$
...
Modelling a permanent magnet synchronous motor (Rolls Royce)
...
![width:650](smart_2.png)

</div>
<div>

![width:400px](smart.png)
